...
I = 1.0e-46 # Moment of inertia of molecule in kg*m^2
T = 300.0 # Temperature in K
mu = 1.0e-30 # Electric dipole moment of molecule in C*m

# Define a function to calculate the rigid rotor energy
def E(J):
    return hbar**2/(2.0*I)*J*(J+1.0)
...
def Z(T):
    ...
    while True:
        ...
        Z += (2.0*J+1.0)*np.exp(-E(J)/(kB*T))
        # Stop when the contribution is negligible
        if (2.0*J+1.0)*np.exp(-E(J)/(kB*T)) < 1.0e-10:
            break
        # Increment J by 1
        J += 1
    return Z
...
def rho0(J,M,Jp,Mp,T):
    ...
    cJM = np.sqrt((2.0*J+1.0)*np.exp(-E(J)/(kB*T))/Z(T))*(M==0)
    cJpMp = np.sqrt((2.0*Jp+1.0)*np.exp(-E(Jp)/(kB*T))/Z(T))*(Mp==0)
...
def Efield(t):
    ...
    E0 = 1.0e5 # Peak electric field in V/m
    omega = 1.0e15 # Carrier frequency in rad/s
    tau = 1.0e-12 # Pulse duration in s
    return E0*np.exp(-t**2/(2.0*tau**2))*np.cos(omega*t)

# Define a function to calculate the matrix element of the electric dipole operator
def muJM(J,M,Jp,Mp):
    # Use the Wigner-Eckart theorem
    if M == Mp:
        return mu*np.sqrt((2.0*J+1.0)*(2.0*Jp+1.0))*np.sqrt((J+M)*(J-M))*np.sqrt((Jp+Mp)*(Jp-Mp))/(2.0*(2.0*J-1.0))
    else:
        return 0
        ...
        for M in range(-J,J+1):
            for Jp in range(Jmax+1):
                for Mp in range(-Jp,Jp+1):
                    ...
                    drho_dt = -1j/hbar*(H1I(J,M,Jp,Mp,t)*rho_matrix[i,j]-rho_matrix[i,j]*H1I(Jp,Mp,J,M,t))
